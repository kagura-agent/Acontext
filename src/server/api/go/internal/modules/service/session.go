...
func NewSessionService(sessionRepo repo.SessionRepo, sessionEventRepo repo.SessionEventRepo, assetReferenceRepo repo.AssetReferenceRepo, log *zap.Logger, s3 *blob.S3Deps, publisher *mq.Publisher, cfg *config.Config, redis *redis.Client, materialSvc MaterialService) SessionService {
...
func (s *sessionService) StoreMessage(ctx context.Context, in StoreMessageInput) (*model.Message, error) {
	...
	if s.redis != nil {
		if err := s.cachePartsInRedis(ctx, asset.SHA256, parts, in.UserKEK); err != nil {
