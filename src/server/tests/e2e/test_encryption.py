...
async def create_test_project_encrypted(conn) -> ProjectCredentials:
    """Create a test project with encryption_enabled = true."""
...
async def plain_project(db_conn) -> AsyncGenerator[ProjectCredentials, None]:
    """Project with encryption_enabled = false (default)."""
...
@pytest.mark.asyncio
async def test_enable_encryption_on_plain_project(db_conn, plain_project):
...
async def test_enable_encryption_already_enabled(db_conn, encrypted_project):
    """POST /admin/v1/project/encrypt when already encrypted → 400."""
    async with httpx.AsyncClient() as client:
